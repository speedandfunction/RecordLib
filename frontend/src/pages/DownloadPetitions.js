...
import AttorneyHolder from "frontend/src/components/AttorneyHolder";
import AddAttorney from "frontend/src/components/AddAttorney";
  ...
  return (
    <Container>
      {petitionPackage.hasOwnProperty("path") ? (
        <a href={petitionPackage.path} download>
          {" "}
          Download the petition package{" "}
        </a>
      ) : (
        ""
      )}
      <form onSubmit={submitGetPetitions}>
        <h4>
          {" "}
          There are a few other pieces of information you need to generate
          petitions.{" "}
        </h4>
...
const DownloadPetitionsWrapper = connect(
  mapStateToProps,
  mapDispatchToProps
)(DownloadPetitions);
