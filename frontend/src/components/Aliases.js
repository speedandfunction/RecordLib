...
import AliasHolderWrapper from "frontend/src/components/AliasHolder";
import AddAliasWrapper from "frontend/src/components/AddAlias";
import ShowHideList from "frontend/src/components/ShowHideList";
...
function Aliases(props) {
  ...
  const aliasesRendered = aliases.map((aliasId) => {
    return (
      <AliasHolderWrapper key={aliasId} aliasId={aliasId} editing={editing} />
    );
  });
