        ...
        {serviceAgencies.result.map((idx) => {
          return (
            <ListItem key={serviceAgencies.entities[idx].id}>
              ...
              <IconButton
                aria-label="delete"
                onClick={createHandleDeleteSA(serviceAgencies.entities[idx].id)}
              >
                <DeleteIcon />
              </IconButton>
            </ListItem>
...
function mapDispatchToProps(dispatch) {
  return {
    updateServiceAgency: (id, value) =>
      dispatch(updateServiceAgency(id, value)),
    createNewServiceAgency: (serviceAgency) =>
      dispatch(createNewServiceAgency(serviceAgency)),
