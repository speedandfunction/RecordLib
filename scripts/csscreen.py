...
from RecordLib.sourcerecords.docket.parse_pdf import parse_cp_pdf_text
...
def communicate_results(sourcerecords: List[SourceRecord], analysis: Analysis, output_json_path: str, output_html_path: str, email_address) -> None:
    """
...
@click.option("--date-format", help="Date format", default=r"%m/%d/%Y", required=False, show_default=True)
...
@click.option("--output-dir", "-od", help="Path to a directory to write downloaded pdfs.", default=None)
@click.option("--log-level", help="Log Level", default="INFO", required=False, show_default=True)
def name(first_name, last_name, dob, date_format, output_json, output_dir, output_html, email, log_level):
    """
    ...
    with tempfile.TemporaryDirectory() as td:
        for case in search_results:
            for source_type in ["docket_sheet", "summary"]:
                try:
                    resp = requests.get(case[f"{source_type}_url"], headers={"User-Agent": "CleanSlateScreener"})
    ...
    sourcerecords = list()
    crecord = CRecord(person=Person(first_name=first_name, last_name=last_name, date_of_birth=dob))
    for case in search_results:
...
@cli.command()
@click.option("--input-dir", "-i", help="Path to directory containing docket_sheet files to parse.")
@click.option("--output-json", "-oj", help="Path to ouput the json data", default=None)
@click.option("--output-html", "-oh", help="Path to write html output", default=None)
@click.option("--email", "-e", help="Email address to send to (optional)", default=None)
@click.option("--log-level", help="Log Level", default="INFO", required=False, show_default=True)
def dir(input_dir, output_json, output_html, email, log_level):
    ...
    for df in docket_files:
        ...
        source_records.append(SourceRecord(get_text_from_pdf(os.path.join(input_dir, df)), parser))
