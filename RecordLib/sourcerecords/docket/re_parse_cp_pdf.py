...
from RecordLib.sourcerecords.parsingutilities import get_text_from_pdf, date_or_none, money_or_none
import logging
import re
...
def parse_person(txt: str) -> Tuple[Person, List[str]]:
    ...
    errs = []
    defendant_name, d_errs = find_pattern("defendant_name", r"^Defendant\s+(?P<last_name>.*), (?P<first_name>.*)",txt,re.M)
    ...
    defendant_dob, dob_errs = find_pattern("date_of_birth", r"Date Of Birth:?\s+(?P<date_of_birth>\d{1,2}\/\d{1,2}\/\d{4})",txt)
    if defendant_dob is not None:
        person.date_of_birth = date_or_none( defendant_dob.group("date_of_birth") )
    else:
        errs.extend(dob_errs)

    defendant_info_section, d_section_errs = find_pattern("defendant_info", r"DEFENDANT INFORMATION(?P<defendant_info>.*)\s+CASE PARTICIPANTS", txt, re.DOTALL)
    if defendant_info_section is not None:
        defendant_info_text = defendant_info_section.group("defendant_info")
        alias_search, a_errs = find_pattern("aliases", r"Alias Name\s*\n+(?P<aliases>(.+\s*\n*)*)",defendant_info_text)
        if alias_search is not None:
            person.aliases = [a.strip() for a in alias_search.group("aliases").split("\n") if len(a) > 0]
        else:
            errs.extend(a_errs)


        addr_search, addr_errs = find_pattern("address", r"City/State/Zip:\s*(?P<addr>.*)\s*",defendant_info_text)
        if addr_search is not None:
            person.address = addr_search.group("addr")
        else:
...
def parse_charges(txt: str) -> Tuple[Optional[List[Charge]],List[str]]:
    ...
    disposition_section_searcher = re.compile(r"(?:.*\s+)DISPOSITION SENTENCING/PENALTIES\s*\n(?P<disposition_section>(.+\n+(?=[A-Z ]+))+.*)")
    errs = []
    ...
    for disposition_section in disposition_sections:
        ...
        for idx, ln in enumerate(section_lines):
            ...
            if charge_line_search is not None:
                ...
                charge_overflow_search = re.search(r"^\s+(?P<offense_overflow>\w+\s*\w*)\s*$", section_lines[idx+1], re.I)
                if charge_overflow_search is not None:
                    offense += (" " + charge_overflow_search.group('offense_overflow').strip())
                    idx_copy += 1
                try:
                ...
                disp_date_search = re.search(r"(.*)\s(?P<disposition_date>\d{1,2}\/\d{1,2}\/\d{4})",section_lines[next_line_index])
                while re.search(r"(.*)\s(?P<disposition_date>\d{1,2}\/\d{1,2}\/\d{4})",section_lines[next_line_index]):
                    disp_date_search = re.search(r"(.*)\s(?P<disposition_date>\d{1,2}\/\d{1,2}\/\d{4})",section_lines[next_line_index])
                    next_line_index += 1
                ...
                if disp_date_search is not None:
                    charge.disposition_date = date_or_none(disp_date_search.group('disposition_date'))
                    if charge.disposition_date is None:
                        errs.append(f"For the offense, {charge.sequence}/ {offense}, we found, but could not parse, the disposition date: {disp_date_search.group('disposition_date')}")
                charges.append(charge)
    charges = Charge.reduce_merge(charges)
    missing_disposition_dates = [f"Could not find disposition date for {c.sequence} / {c.offense} with disposition {c.disposition}" for c in charges if c.disposition_date is None]
    return charges, errs
...
def parse_case(txt: str) -> Tuple[Case,List[str]]:
    ...
    docket_number_search, dn_errs = find_pattern("docket_number", r"Docket Number:\s+(?P<docket_number>(MC|CP)\-\d{2}\-(\D{2})\-\d*\-\d{4})", txt)
    if docket_number_search is not None:
        case.docket_number = docket_number_search.group("docket_number")
    else:
        errs.extend(dn_errs)

    otn_search, otn_errs = find_pattern("otn", r"OTN:\s+(?P<otn>\D(\s)?\d+(\-\d)?)", txt)
    ...
    status_search, status_search_errs = find_pattern("status",r"case status:\s+(?P<status>(?:\w+\s)+)", txt, re.I)
    ...
    cty_search, cty_errs = find_pattern("county", r"\sof\s(?P<county>\w+)\sCOUNTY",txt, re.I)
    if cty_search is not None:
        case.county = cty_search.group('county')
    else:
        errs.extend(cty_errs)

    complaint_date_search, cd_errs = find_pattern("complaint_date",r"Complaint Date:\s+(?P<complaint_date>\d{1,2}\/\d{1,2}\/\d{4})", txt)
    if complaint_date_search is not None:
        ...
        if complaint_date is not None:
            ...
            errs.append(f"Found complaint date, but could not understand the date format: {complaint_date_search.group('complaint_date')}")
    else:
        errs.extend(cd_errs)

    arrest_date_search, arrest_date_errs = find_pattern("arrest_date", r"Arrest Date:\s+(?P<arrest_date>\d{1,2}\/\d{1,2}\/\d{4})", txt)
    if arrest_date_search is not None:
        ...
        if arrest_date is not None:
            ...
            errs.append(f"Found arrest date but could not understand the date format: {arrest_date_search.group('arrest_date')}")
    else:
    ...
    if disp_date_search is not None:
        ...
        if disp_date is not None:
            ...
            errs.append(f"Found disposition date, but could not understand date format: {disp_date_search.group('disposition_date')}")
    ...
    judge_assignment_pattern = r"Judge Assigned:\s+(?P<judge_assigned>.*)\s+(Date Filed|Issue Date):"
    judge_assigned_search, judge_assigned_errs = find_pattern("judge_assigned", judge_assignment_pattern ,txt)
    if judge_assigned_search is not None:
        ...
        if judge_overflow_search is not None:
            judge_assigned += (" " + judge_overflow_search.group("judge_overflow").strip())
    ...
    final_issue_auth_search, final_issue_auth_errs = find_pattern("final_issuing_authority", r"Final Issuing Authority:\s+(?P<judge_name>.*)",txt)
    ...
    dc_search, dc_errs = find_pattern("dc",r"District Control Number\s+(?P<dc>\d+)", txt)
