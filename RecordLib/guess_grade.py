...
def guess_grade(c: Charge) -> Tuple[str, float]:
    ...
    if c.get_statute_subsections() != "":
        ...
        cur.execute(query, (c.get_statute_chapter(), c.get_statute_section()))
    else:
        query = ("SELECT SUM(Percent) as P FROM crimes_w_subsection WHERE title='%s' AND section='%s' " +
                 "AND subsection like '%s%'")
        cur.execute(query, (c.get_statute_chapter(), c.get_statute_section(), c.get_statute_subsections()))
    breakpoint()
    return ("", 0)